...
[`prompts/`](../prompts/); each agent class loads its prompt at
construction time.

### 9.1 Per-agent prompt mapping
...
image bytes, and for short-form agents whose prompt is short enough to
live beside the parser.

### 9.2 Shared context file

[`prompts/shared/context.md`](../prompts/shared/context.md) consolidates 11
runtime reference fragments from the original working tree into one file
